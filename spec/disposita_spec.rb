...
#
# These examples verify schema construction through Disposita.define_schema and guard
...
RSpec.describe Disposita do
  describe ".define_schema" do
    it "returns an independent schema" do
      schema = described_class.define_schema(:example) do
    ...
    it "requires a definition block" do
      expect { described_class.define_schema(:example) }.to raise_error(Disposita::SchemaError)
    end
  end
  ...
  it "does not read files or environment values when declaring a schema" do
    ...
    schema = described_class.define_schema(:app) { setting :name, type: String, env: "APP_NAME" }
